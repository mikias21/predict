...
class MLPRegression(object):
    ...
    def make_prediction(self):
        self.prediction = auxilary.make_prediction(self.x, self.y, self.x_train, self.x_test, self.y_train, self.regressor)
        return str(self.prediction.tolist()), self.data.columns
    
    def make_single_prediction(self):
        # Create new data frame
        single_prediction = auxilary.make_single_prediction(self.data.columns, self.indivisual_inputs, self.regressor)
        return str(single_prediction.tolist())
    
    def get_success_rate(self):
        mae, mse, r2score, accuracy, confusion = auxilary.get_seccess_rate(self.y, self.y_test, self.prediction)
