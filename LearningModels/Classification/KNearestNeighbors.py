...
class KNeighborsClassification(object):
    ...
    def __init__(self, data, dependant_var, indivisualInputs,  n_neighbours, metric, p, unwanted_cols = None, testsize = None):
        ...
        self.n_neighbours = int(float(n_neighbours)) if n_neighbours != 'null' else 5
        self.metric = metric if metric != 'null' and metric != 'undefined' else 'minkowski'
        self.p = int(float(p)) if len(p) != 0 else 2
        self.classifier = KNeighborsClassifier(n_neighbors=self.n_neighbours, metric=self.metric, p=self.p)
        self.scaler = None
